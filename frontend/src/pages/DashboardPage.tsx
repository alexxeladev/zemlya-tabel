  ...
  return (
    ...
    >
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-3 xl:grid-cols-6">
        ...
        {/* Нераспределённый остаток (task_distribution_base_fix) — ОТДЕЛЬНЫЙ
            показатель, не тот же самый. Суммы распределения по юрлицам
            округляются ВНИЗ до 1000 ₽, поэтому по каждой строке остаётся
            0…999 ₽, которые не приписываются ни одной компании. Всегда ≥ 0 —
            знака, в отличие от «Эффекта округления», у него не бывает. */}
        <KpiCard
          label="Нераспределённый остаток"
          value={formatMoney(p.unallocated_remainder ?? '0', { showZero: true })}
          accent="text-gray-900"
          hint={
            'Затраты по юрлицам разносятся круглыми тысячами: «Итого начислено» ' +
            'минус разнесённое, 0…999 ₽ на сотрудника. Никому не приписывается. ' +
            'Не путать с «Эффектом округления» — тот про округление выплаты.'
          }
        />
      </div>
