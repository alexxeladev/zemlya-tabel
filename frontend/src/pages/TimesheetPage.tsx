...
export function TimesheetPage() {
  ...
  /**
   * Количественный показатель (АРМ у ИТ, Заявки у HR) — только по отделам, чьи
   * строки реально остались в выборке. Бэк отдаёт показатель по всем отделам с
   * флагом и о клиентских фильтрах (юрлицо + фильтры колонок) не знает, поэтому
   * отбор делается здесь и ОТ УЖЕ ОТФИЛЬТРОВАННЫХ СТРОК — разбирать фильтры
   * повторно значило бы завести вторую копию правила.
   *
   * ОДИН источник на оба места: и блок ввода над таблицей (`QuantityPanel`), и
   * колонки «Распределение» в хвосте строк (`distributionOn`) считают видимость
   * отсюда. Двумя условиями они неминуемо разъехались бы — и на экране остался
   * бы либо блок без колонок, либо колонки прочерков без блока.
   */
  const shownQuantities = useMemo(
    () =>
      quantitiesForVisibleDepartments(
        data?.quantities ?? [],
        shownRows.map((r) => r.position.department_id)
      ),
    [data, shownRows]
  );
  ...
  // ── Колонки «Распределение» (заявки у HR, АРМ у ИТ) ──
  // Показываются только в табеле отдела «по количественному показателю» и только
  ...
  //
  // Отбор отделов — ТОТ ЖЕ `shownQuantities`, что уходит в блок ввода, а не
  // своя копия правила: иначе блок и колонки разъехались бы при первой же
  // правке фильтров. Нет в выборке ни одного отдела с показателем — колонок нет
  // вовсе, а не столбцы прочерков (ровно как нет и блока ввода).
  // Своё у колонок только ДВА дополнительных условия, и они не про отбор
  // отделов: деньги (`canSeeMoney`) и то, что показатель за месяц заведён
  // (`!is_empty`) — блок ввода, наоборот, обязан показываться и на пустом
  // показателе, иначе цифры некуда вводить.
  const distributionOn = canSeeMoney && shownQuantities.some((a) => !a.is_empty);
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden min-w-0">
      ...
      {/* ───── Количественный показатель отдела (заявки у HR, АРМ у ИТ) ─────
           ...
           количественному показателю», И ТОЛЬКО тех, чьи строки остались в
           текущей выборке (юрлицо + фильтры колонок): иначе таблицы ввода АРМ
           и Заявок висели над табелем отдела, к показателю не имеющего
           отношения. Ни одного такого отдела в выборке — блока нет вовсе. */}
      <QuantityPanel
        quantities={shownQuantities}
        companies={data.companies}
        year={year}
        month={month}
        canEdit={canSeeMoney}
