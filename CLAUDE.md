...
- **«Рабочий день по графику»** — `app/services/work_schedule.py`, единственный источник правды (используют и payroll, и Excel-экспорт): `is_schedule_work_day(schedule, date, calendar_data) -> bool | None` и обёртка `is_off_schedule_day(...) -> bool`.
  - weekday-график (`schedule_type != "shift"`): рабочий, если день недели входит в рабочие дни графика ИЛИ календарь объявил день рабочим (перенос — «рабочая суббота»). Явного поля с днями недели в `Schedule` нет: набор выводится из имени «N/M» при N+M=7 (5/2 → Пн–Пт, 6/1 → Пн–Сб), иначе дефолт Пн–Пт. Появится поле `work_weekdays` — оно имеет приоритет.
  - cyclic-график (`schedule_type == "shift"`, 2/2, 3/3): рабочий по циклу от даты старта, календарь не влияет. **Поля даты старта в модели пока нет** (ждёт задачу сменных графиков), поэтому функция возвращает `None` = «определить нельзя»; читается `cycle_start_date` через getattr. `None` трактуется как «не вне графика» (лишней доплаты не возникает), а payroll для shift и так даёт `is_calculable=False`.
...
- `norm_days` (рабочих дней по календарю) и `fact_days` (дней с часами) — справочные, в деньгах не участвуют. **Месячная норма часов остаётся календарной** (`norm_hours_for_period`) — график меняет только оплату отработанных часов.
