...
- **Переработка ПОМЕСЯЧНО** (задача 3.11b п.0, уточнение финдира — откат правки 3.9 «по дням»): `переработка_часов = max(0, факт_будних_часов − месячная_норма)`. Праздничные/выходные часы — отдельная категория, в факт будних и в переработку НЕ входят.
- **База оклада** от зачётных будних часов: `base = rate × min(1, зачётные_будние / norm)`, где зачётные = `min(факт_будних, norm)`. Праздничные/выходные часы в базу НЕ входят.
