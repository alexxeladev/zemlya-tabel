...
- **Переработка ПО ДНЯМ** (task_overtime_daily — финальное решение, откат помесячного варианта 3.11b п.0): для каждого дня `max(0, факт_дня − дневная_норма)`, суммируется за месяц. Часы всех компаний в дне складываются. Дневная норма = `hours_per_shift`, у сокращённого дня `hours_per_shift − 1`, у **дня вне графика** 0. Недоработка одного дня НЕ гасит переработку другого. Часы вне графика — отдельная категория, в зачётные часы и в переработку НЕ входят. Хелперы: `daily_norm_hours` / `daily_overtime_hours` в `app/services/payroll.py` (оба принимают `schedule`).
...
- **Оплата считается от ГРАФИКА сотрудника, а не от производственного календаря** (task_schedule_based_pay):
  - работа в **рабочий день графика** — по окладу (×1), **даже если это календарный праздник**: 12 июня в пятницу для 5/2 — обычный рабочий день, доплаты нет;
  - превышение дневной смены в рабочий день графика — переработка (как раньше);
  - **выход в свой выходной ПО ГРАФИКУ** — отдельная категория «вне графика»: ВСЕ часы такого дня оплачиваются per-employee (`weekend_pay_type = coefficient` → `часы × (rate/norm) × коэффициент`, 0 = не платить, дефолт 1.5; `fixed_rate` → `часы × фикс_ставка`, не зависит от оклада). Поля `weekend_pay_type / weekend_coefficient / weekend_fixed_rate` в Employee — механизм прежний, изменился только триггер.
...
- **Имена полей `holiday_hours` / `holiday_amount` оставлены прежними** (API, UI, ведомость, Excel), но семантика теперь «часы/оплата ВНЕ ГРАФИКА», а не «праздники по календарю». В UI и Excel подписи — «Вне граф.» / «Вне графика».
...
- Распределение по компаниям (`CompanyBreakdown`): base/overtime ₽ — пропорционально часам; holiday ₽ — пропорционально часам вне графика по компании. Поля per-company: `hours`, `percent`, `overtime_hours`, `holiday_hours`, `base_amount`, `overtime_amount`, `holiday_amount`, `total`. `holiday_hours` — точные (где отработаны), `overtime_hours` — пропорционально часам (метод наибольших остатков, сумма = итог).
