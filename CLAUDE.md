...
- **Распределение затрат по юрлицам в %** — управленческое. Гибрид как у займа:
  - дефолт в карточке — `EmployeeCompanyShare (employee_id, company_id, percent)`, валидация суммы ≈100% (99–101);
  - помесячное переопределение — `CompanyShareOverride (employee_id, company_id, year, month, percent)`, строки существуют ТОЛЬКО при правке в ведомости; есть хоть одна за месяц → берётся весь набор override, иначе дефолт из карточки.
- **Авто-распределение по часам, если ручной % не задан** (task_distribution_fix): если сумма ручных % = 0 (ни override, ни дефолт в карточке) → «Итого начислено» распределяется пропорционально фактическим часам сотрудника по компаниям из табеля (`_auto_shares_by_hours`, тот же метод наибольших остатков). Ручной % (>0) перекрывает авто; очистка возвращает к авто. Нет часов и нет % → вся сумма на `default_company`. Флаг `is_auto_distributed` в `StatementRow` (Pydantic-поле ответа, не колонка БД). Excel: авто-строки не помечаются «≠100%», примечание «распределено по часам (авто)». Фронт: авто-строки не префилл, плейсхолдер вычисленного %, пунктир/серый курсив.
