...
- **ТРИ непересекающиеся категории часов** — `day_category(schedule, date, calendar_data)` в `work_schedule.py`, единственный источник правды (её же используют Excel-экспорт и ведомость):
  ...
  - **`holiday`** — день в план НЕ входит и является нерабочим **праздничным** по календарю: ВСЕ часы дня — праздничные, per-employee `holiday_pay_type / holiday_coefficient / holiday_fixed_rate`, **дефолт коэффициента 2.0** (ТК ст. 153 — не менее двойного);
