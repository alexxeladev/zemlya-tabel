...
- **Переработка ПО ДНЯМ** (task_overtime_daily — финальное решение, откат помесячного варианта 3.11b п.0): для каждого дня `max(0, факт_дня − дневная_норма)`, суммируется за месяц. Часы всех компаний в дне складываются. Дневная норма = `hours_per_shift`, у сокращённого дня `hours_per_shift − 1`, у выходного/праздника 0. Недоработка одного дня НЕ гасит переработку другого. Праздничные/выходные часы — отдельная категория, в факт будних и в переработку НЕ входят. Хелперы: `daily_norm_hours` / `daily_overtime_hours` в `app/services/payroll.py`.
- **База оклада** от зачётных будних часов: `base = rate × min(1, зачётные_будние / norm)`, где зачётные = `Σ min(факт_дня, дневная_норма)` (= факт_будних − переработка). Праздничные/выходные часы в базу НЕ входят.
...
- `total_amount` = оклад + переработка + праздничные + отпускные + больничные. `breakdown_by_company` остаётся **только по рабочим категориям** — разнесение отпускных по юрлицам делает ведомость (проценты каскада), само отсутствие к юрлицу не привязано.
...
- **Итого начислено** в ведомости = `base_salary (оклад+праздничные) + переработка + премия + KPI`; **К выплате** = Итого начислено − удержания (= `net_payout` из табеля).
