...
class TestNotOnPost:
    ...
    def test_official_payout_needs_a_post(self, db_session, rodionov):
        """Место, которое НИ РАЗУ не ставили на пост, выплат не порождает.

        С task_official_payout_debt межвахтовый месяц БЕЗ поста выплату даёт —
        банк платит по договору, и она уходит в долг. Но отсчёт начинается с
        первого месяца на посту (решение заказчика 25.09.2026): у места, которое
        на пост не ставили вовсе, истории нет и начислять не с чего. Иначе
        позиция, заведённая в справочнике, копила бы долг с даты приёма.
        """
        _official(rodionov.primary_position)
        db_session.commit()
        row = build_payroll_statement(db_session, [rodionov], [], YEAR, MONTH).rows[0]
    ...
    def test_payout_covers_the_whole_month_of_a_posted_place(
        ...
        # Во второй половине начислений нет: касса не выдаёт ничего, а переплата
        # 12 615 становится ДОЛГОМ следующих половин (task_official_payout_debt).
        # До неё половина показывала минус −12 615, и он никуда не переносился.
        assert row.halves[1].net_payout == Decimal("0")
        assert row.official_debt_after == Decimal("12615.00")
