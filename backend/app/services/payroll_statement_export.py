...
# Пояснения к заголовкам, состав которых не очевиден из названия
# (task_overtime_columns). Заголовки образца трогать нельзя — в шаблоне финдира
# зафиксированы буквы колонок и их подписи.
_HEADER_HINTS = {
    "Кол-во переработки, час": (
        "Сверхурочные часы плюс часы работы в выходные и праздники по графику "
        "— то есть всё, что оплачено по коэффициенту. Ровно эти часы дают "
        "сумму в колонке «Сумма ПЕРЕРАБОТКи»."
    ),
    "Сумма ПЕРЕРАБОТКи": (
        "Оплата сверхурочных плюс оплата работы в выходные и праздники по "
        "графику."
    ),
    "Начислено, оклад": (
        "Только оплата обычных часов. Выходные и праздничные — в колонках "
        "переработки."
    ),
}
...
def generate_statement_excel(statement: PayrollStatementRead) -> bytes:
    ...
    for i, title in enumerate(headers):
        ...
        c.border = border
        c.fill = header_fill
    # Что именно лежит в колонках переработки и оклада (task_overtime_columns):
    # состав менялся, а заголовки образца — нет, поэтому пояснение уходит
    # примечанием к самим заголовкам, чтобы выгрузка объясняла себя сама.
    for title, hint in _HEADER_HINTS.items():
        ws.cell(row=header_row, column=_FIRST_COL + _FIXED_HEADERS.index(title)) \
            .comment = Comment(hint, "Расчёт ЗП", height=90, width=300)
