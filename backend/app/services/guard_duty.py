...
def copy_previous_period(
    ...
    **Смены НЕ проставляются** (решение заказчика 25.09.2026): переносится
    только состав — кто на каком посту и по какой ставке. Раньше копирование
    отмечало ВСЕ дни месяца, и при вахтовом цикле 15/15 это давало вдвое больше
    смен, чем бывает: на деве октябрь получил 2 201 смену против 1 373 в
    сентябре-источнике, у ГБР выходило 139 500 ₽ вместо ~67 500 ₽. Лишние дни
    приходилось снимать руками по каждой строке, а незамеченные уезжали в
    ведомость. Отметить смены — одна протяжка мышью по строке.
