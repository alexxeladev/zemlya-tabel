...
class PayrollTotalsRead(BaseModel):
    ...
    holiday: Decimal
    # Эффект округления «к выплате» до 1000 ₽ — сумма хвостов всех сотрудников
    # за период (task_payout_rounding). Знак любой: + осело в пользу компании
    # (округлили вниз), − компания доплатила до тысячи (округлили вверх).
    rounding_effect: Decimal = Decimal("0")
    # Нераспределённый остаток — Σ (Итого начислено − Σ распределения по
    # юрлицам) за период (task_distribution_base_fix). ДРУГОЙ показатель, не
    # путать с rounding_effect: тот про округление ВЫПЛАТЫ и бывает любого
    # знака, этот про округление РАСПРЕДЕЛЕНИЯ вниз до 1000 ₽ и всегда ≥ 0.
