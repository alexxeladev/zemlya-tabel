...
class StatementRow(BaseModel):
    ...
    # Переработка ведомости (task_overtime_columns) — ОДНИ И ТЕ ЖЕ категории в
    # часах и в рублях: сверхурочные ПЛЮС работа в выходные/праздники по
    # графику, то есть всё, что оплачено по коэффициенту. Число часов здесь —
    # ровно то, из которого посчитана сумма рядом.
    # Дельты «факт − норма» в ведомости нет и быть не должно: она схлопывает
    # переработку с недоработкой и в расчёте не участвует.
    overtime_hours: Decimal
    overtime_amount: Decimal

    # Начислено, оклад — ТОЛЬКО оплата обычных часов. Выходные и праздничные
    # уехали в колонки переработки выше; «Итого начислено» от этого не меняется.
    base_salary: Decimal
    ...
    # loan_note заполняется у РУЧНОЙ правки удержания по займу (своего поля
    # обоснования у займа в модели нет, есть лишь плановая доля) и когда
    # начисления не хватило на плановую долю (п.5.1).
    ...
    # сколько долга пришло из прошлых половин, сколько погашено в этом месяце и
    # сколько уехало дальше. На «Итого начислено», налог и базу распределения не
    # влияет — только на «К выплате».
    official_debt_before: Decimal = Decimal("0")
    official_debt_after: Decimal = Decimal("0")
    official_debt_repaid: Decimal = Decimal("0")
    # Объяснение для бухгалтера, почему «Итого начислено − Удержано» не равно
    # «К выплате»: часть ушла на гашение переплаты прошлых половин или,
    # наоборот, не выплачена и уехала долгом дальше.
    official_debt_note: str | None = None
